...
function mapSanitized(
  sanitizedEntity: GetAttributesValues<"api::category.category">[]
) {
    ...
    async find(ctx) {
      ...
      try {
        ...
        const sanitizedEntity = (await this.sanitizeOutput(
          entity.results,
          ctx
        )) as GetAttributesValues<"api::category.category">[];

        if (navigation.show_empty_categories === false) {
          const count = await Promise.all(
            sanitizedEntity.map(async ({ uid }) => ({
              uid,
